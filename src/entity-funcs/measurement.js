...
export function addSegmentLabel(viewer, start, end) {
...
export function addTotalLabel(viewer, cartesian, totalDistance) {
  return viewer.entities.add({
    position: cartesian,
    label: {
...
export function addAreaLabel(viewer, cartesian, area) {
  return viewer.entities.add({
    position: cartesian,
    label: {
...
export function finishDistance(viewer, points) {
  const labels = [];
  if (points.length < 2) {
    return labels;
  }
  ...
  for (let i = 1; i < points.length; i++) {
    labels.push(addSegmentLabel(viewer, points[i - 1], points[i]));
  }

  const total = getTotalDistance(points);
  labels.push(addTotalLabel(viewer, points[points.length - 1], total));

  return labels;
}

export function finishArea(viewer, points) {
  if (points.length < 3) {
    return [];
  }
  ...
  return [addAreaLabel(viewer, center, area)];
}